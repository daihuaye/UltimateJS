	...
	function build() {
		...
		svg.Element.text = function(node) {
			...
			if (node != null) {
				...
				for (var i=0; i<node.childNodes.length; i++) {
					var childNode = node.childNodes[i];
					if (childNode.nodeType == 1) { // capture tspan and tref
													// nodes
		...
		svg.load = function(ctx, url) {
			...
			// viking
			if (!(svg.currentFiles)){
				svg.currentFiles = new Array();
			}
			if (!(svg.currentFiles[url])){
				svg.currentFiles[url] = svg.ajax(url);
			}
			svg.loadXml(ctx, svg.currentFiles[url]);
		}
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			// viking
			// console.log("Size is (%d:%d)",e.attribute('width',
			// true).value,e.attribute('height', true).value);
			...
			var draw = function() {
				...
				if (svg.opts['ignoreDimensions'] != true) {
					...
					if (e.style('height').hasValue()) {
					 ...
					 // viking
					 console.log("Canvas size is (%d:%d)",ctx.canvas.width,ctx.canvas.height);
...
// console.log("Draw takes %d",(t2-t1));
			}
